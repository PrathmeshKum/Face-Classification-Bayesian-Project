...
mean_face1=reshape(mean_face1,[40 30 1]);
mean_face2=zeros(40,30,3);
mean_face2=uint8(mean_face2);
mean_face2(:,:,1)=0;
mean_face2(:,:,2)=mean_face1;
mean_face2(:,:,3)=0;
figure;
subplot(2,2,1);
showIm=mean_face2;
...
covar_face1=reshape(covar_face1,[40 30 1]);
covar_face4=zeros(40,30,3);
covar_face4=uint8(covar_face4);
covar_face4(:,:,1)=0;
covar_face4(:,:,2)=covar_face1;
covar_face4(:,:,3)=0;
subplot(2,2,3);
showIm=covar_face4;
imshow(showIm);
...
mean_bg1=reshape(mean_bg1,[40 30 1]);
mean_bg2=zeros(40,30,3);
mean_bg2=uint8(mean_bg2);
mean_bg2(:,:,1)=0;
mean_bg2(:,:,2)=mean_bg1;
mean_bg2(:,:,3)=0;
subplot(2,2,2);
...
covar_bg1=reshape(covar_bg1,[40 30 1]);
covar_bg4=zeros(40,30,3);
covar_bg4=uint8(covar_bg4);
covar_bg4(:,:,1)=0;
covar_bg4(:,:,2)=covar_bg1;
covar_bg4(:,:,3)=0;
subplot(2,2,4);
showIm=covar_bg4;
imshow(showIm);
...
%bg_img_test=mvnpdf(transpose(face_matrix),transpose(mean_bg),covar_bg2);

%fc_img_test=mvgd1(test_face_matrix,mean_face,covar_face2,fc_test_image_num,1200);
%fc_img_test_1=mvgd1(test_face_matrix,mean_bg,covar_bg2,fc_test_image_num,1200);
...
%fc_img_test=mvnpdf(transpose(background_matrix),transpose(mean_face),covar_face2);

 %bg_img_test=mvgd1(test_background_matrix,mean_bg,covar_bg2,bg_test_image_num,1200);
 %bg_img_test_1=mvgd1(test_background_matrix,mean_face,covar_face2,bg_test_image_num,1200);
...
path=[directory '\rgb_single_model_data.mat'];
save(path);